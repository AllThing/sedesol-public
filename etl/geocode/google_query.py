...
def get_constraints(geo_info):
    ...
    """
    return {"country": "MX",
            "administrative_area_1": geo_info["entidad_federativa"],
            "administrative_area_2": geo_info["municipio"],
            "locality": geo_info["localidad"],
            "sublocality": geo_info["asen"]}
...
def get_street_info(street_info):
    ...
    :rtype dict
    """
    search_str = {"localidad": street_info["localidad"],
                  "num_ext": "",
                  "vial": "",
                  "intersect": []}
    if not isnull(street_info["num_ext"]):
        search_str["num_ext"] = street_info["num_ext"]
    if not isnull(street_info["vial"]):
        search_str["vial"] = street_info["vial"]
        if not isnull(street_info["vial_1"]):
            cur_str = "'%s' & '%s'" % (street_info["vial"], street_info["vial_1"])
            search_str["intersect"] = [cur_str]
        if not isnull(street_info["vial_2"]):
            cur_str = "'%s' & '%s'" % (street_info["vial"], street_info["vial_2"])
            search_str["intersect"] += [cur_str]
    return search_str
...
def get_search_strs(street_input, max_query_per_row=2):
    ...
    if len(street_info["num_ext"]) > 0:
        search_str = ["%s %s" % (street_info["vial"], street_info["num_ext"])]
    elif len(street_info["intersect"]) > 0:
        search_str = street_info["intersect"][:max_query_per_row]
    elif len(street_info["vial"]) > 0:
        search_str = [street_info["vial"]]
    else:
        search_str = [street_info["localidad"]]
    return search_str
