/* This script cleans and combines the the raw.manzanas_rurales and
 * raw.manzanas_urbanas during the SEDESOL pipeline. The input are SQL
 * databases containing manzana shapefiles and data from INEGI, one
 ...
 *    - It converts the spatial reference from to WGS (4326)
 */

CREATE TABLE clean.manzanas AS
SELECT 	*,
	ST_Y(ST_CENTROID(geom)) AS latitude,
	ST_X(ST_CENTROID(geom)) AS longitude
FROM (
     SELECT
            cve_mzac AS manzana_id,
            ST_Transform(ST_SetSRID(geom, 8624), 4326) AS geom,
	    SUBSTRING(cve_mzac FROM 1 FOR 9) AS locality_id,
...
UNION
     SELECT cve_mzac AS manzana_id,
            ST_Transform(ST_SetSRID(geom, 8624), 4326) AS geom,
	    SUBSTRING(cve_mzac FROM 1 FOR 9) AS locality_id,
	    nomloc AS locality_name,
	    nom_mun AS municipality_name,
	    nom_ent AS state_name,
	    FALSE AS urban,
	    ...
	    FROM raw.manzanas_rurales) manzanas;
