"""
    This script creates subsetting .sql files for SIFODE and PUB data for the
    beneficiaries for whom we have the manzana_id information. The .sql files
    also join in census data. (SIFODE subsets are used for training models for
    the PUB imputation task. The models are then fit on the PUB subsets to
    predict the values of the six deprivation indicators).
"""

# we need to aggregate the subsets to state-level so that we have enough training
# samples per subset. But state '15' is a lot bigger than other states, and
# we risk running out of memory on that subset, so we break it down into finer
# granularity subsets

state_level_subset_strings = {"0%d" %x: "LIKE '0%d%%'" %x for x in range(1, 10)} 
state_level_subset_strings.update({"%d" %x: "LIKE '%d%%'" %x for x in range(10, 33) if x != 15})
 
finer_granularity_subset_strings = {"150_except_1503" : """LIKE ANY (array['1500%', '1501%',
                                    '1502%', '1504%', '1505%', '1506%',
                                    '1507%', '1508%', '1509%'])""",

                                    "1503": "LIKE '1503%'",

                                    "15_except_150": """LIKE ANY (array['151%',
                                    '152%', '153%', '154%', '155%', '156%',
                                    '157%', '158%', '159%'])"""}

subset_strings = state_level_subset_strings.copy()
subset_strings.update(finer_granularity_subset_strings)

# drop the manzanas that have low geocoding quality
# manzana_id '2801204310683002' is the most popular manzana_id in the geo-coded
# results --- we suspect it is erroneous, and hence throw it out 
bad_manzanas_filter = " AND ".join(["manzana_id is NOT NULL", "manzana_id != '2801204310683002'"])

for subset_id, subset_string in subset_strings.items():
    SQL_SIFODE_STRING = """
    ...
    CREATE TABLE semantic_pub_imputation.sifode_geo_census_subset_%s AS
    (
    ...
    LEFT JOIN manzanas
    USING (manzana_id)
    ); """ %(subset_id, subset_string, bad_manzanas_filter, subset_string, subset_string)



    SQL_PUB_STRING = """CREATE TABLE semantic_pub_imputation.pub_geo_census_subset_%s AS
    (
    WITH manzana_link AS
    (SELECT *
    FROM clean.manzana_link
    ...
    (
    SELECT *,
    /* Need to reconstruct for PUB because locality_id is NULL for people
    * lacking SIFODE */
    CONCAT(payment_entity, payment_municipality, payment_locality) as pub_locality_id 
    FROM
    semantic.semantic_full_backup
    WHERE
    /* Need to subset pub for which we lack indicators */
    indicator_food_deprivation is NULL
    ),

    pub_to_impute_subset AS
    (
    SELECT * 
    FROM
    pub_to_impute
    ...
    pub_subset_manzana_id_joined AS
    (
    SELECT * FROM
    pub_to_impute_subset
    LEFT JOIN manzana_link
    USING (sifode_pub_id)
    WHERE %s 
    ), 
    ...
    localidades AS
    (SELECT
    locality_id AS pub_locality_id,
    locality_name AS locality_name,
    municipality_name AS municipality_name,
    state_name AS state_name,
    urban AS localidad_urban,
    ...
    SELECT * FROM
    pub_subset_manzana_id_joined
    LEFT JOIN localidades
    USING (pub_locality_id)
    LEFT JOIN manzanas
    USING (manzana_id)
    ); """ %(subset_id, subset_string, bad_manzanas_filter, subset_string, subset_string)


    with open("subset_sifode_geo_census_%s.sql" %(subset_id), "w") as g: 
        g.write(SQL_SIFODE_STRING)

    with open("subset_pub_geo_census_%s.sql" %(subset_id), "w") as g: 
        g.write(SQL_PUB_STRING)
